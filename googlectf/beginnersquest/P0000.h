/***
 * Google CTF - Beginner's Quest
 * Functions for solving Problem Set 0000
 *
 * By: jimdiroffii
 *
 * CTF: https://capturetheflag.withgoogle.com/beginners-quest
 *
 * A collection of functions and solutions to the Beginner's Quest problems
 */

#pragma once

#ifndef P0000_H
#define P0000_H

#include <iostream>
#include <string>
#include <iomanip>
#include <vector>
#include <map>

/***
 * Caesar Cipher Solver
 *
 * Just input a string and a shift value to return the plaintext.
 */
inline std::string caesar(const std::string& str, int shift) {
  std::string result{};

  // substitution
  for (auto ch : str) {
    if (isalpha(ch)) {
      auto base = islower(ch) ? 'a' : 'A';
      ch = static_cast<char>((ch - base + shift) % 26 + base);
    }
    result += ch;
  }

  return result;
}


/***
 * Brute force Caesar
 *
 * Used to acquire an unknown shift value. Just iterates through all 25 options
 */
inline void brutus(const std::string& str) {
  for (int i = 1; i <= 25; ++i) {
    std::string decrypted = caesar(str, i);
    std::cout << std::left << "shift " << std::setw(3) << i << ": " << decrypted << '\n';
  }
}

/***
 * Sequence Caesar
 *
 * Executes variable shifts based on a sequence of shift key values
 * The first shift key should coorespond to the first char in cipher
 */
inline std::string seqCaesar(const std::string& cipher, const std::vector<int>& key) {
  std::size_t keyIndex{};
  std::size_t keySize = key.size();

  std::string result{};

  for (auto ch : cipher) {
    if (isalpha(ch)) {
      if (keyIndex >= keySize) {
        keyIndex = 0;
      }

      auto base = islower(ch) ? 'a' : 'A';
      ch = static_cast<char>((ch - base + key[keyIndex++]) % 26 + base);
    }
    result += ch;
  }

  return result;
}

/***
 * Mapped Caesar
 *
 * Each character is mapped to an alternate value
 * Can be used for frequency analysis. Enter guesses for letters as the key
 * and work iteratively through the cipher until all keys are found
 * Unmapped keys are printed back into the result string as-is
 */
inline std::string mapCaesar(const std::string& cipher, const std::map<char, char> key) {
  std::string result{};

  for (auto ch : cipher) {
    if (key.find(ch) != key.end()) {
      result += key.at(ch);
    }
    else {
      result += ch;
    }
  }
  return result;
}

/***
 * Brute Force Transposition Solving
 *
 * To solve a transposition problem, an cipher is input to the
 * transpostionIteration function. The user can be requested to cycle through
 * transpositions (by adding columns), until the proper transposition is found
 *
 * This includes all five functions below:
 *
 * replaceSpecialSequence - used to replace speical characters in input
 * print2Dvector - generic function to print a 2D vector full of char types
 * rearrangeColumns - Once the transposition has been found, reorder the columns
 * buildStringFrom2DArray - as advertised
 * transpositionIteration - Loops through the string, printing all characters
 *  into columns for manual analysis. On each loop, a column is added. The final
 *  version removes the user input, and hardcodes the loop count.
 */

inline void replaceSpecialSequence(std::string& input) {
  const std::string specialSequence = "␣"; // Define the special sequence
  size_t pos = 0;
  while ((pos = input.find(specialSequence, pos)) != std::string::npos) {
    input.replace(pos, specialSequence.length(), "_"); // Replace with underscore
    pos += 1; // Move past the underscore
  }
}

inline void print2DVector(const std::vector<std::vector<char>>& vec) {
  for (const auto& row : vec) {
    std::cout << "{";
    for (size_t i = 0; i < row.size(); ++i) {
      std::cout << row[i];
      if (i < row.size() - 1) {
        std::cout << ",";
      }
    }
    std::cout << "}" << std::endl; // Print each row on a new line
  }
}

inline std::vector<std::vector<char>> rearrangeColumns(const std::vector<std::vector<char>>& matrix) {
  const std::vector<int> newOrder = { 2, 0, 3, 5, 1, 4 };
  std::vector<std::vector<char>> rearranged(matrix.size(), std::vector<char>(matrix[0].size(), ' '));

  for (size_t row = 0; row < matrix.size(); ++row) {
    for (size_t col = 0; col < matrix[0].size(); ++col) {
      rearranged[row][col] = matrix[row][newOrder[col]];
    }
  }

  return rearranged;
}

inline std::string buildStringFrom2DArray(const std::vector<std::vector<char>>& matrix) {
  std::string result;
  for (const auto& row : matrix) {
    for (char ch : row) {
      if (ch != ' ') { // Skip spaces if you don't want them in the final string
        result += ch;
      }
    }
  }
  return result;
}

inline std::string transpositionIteration(std::string input) {
  replaceSpecialSequence(input); // Replace special sequences in the input
  std::vector<std::vector<char>> result{};
  size_t numColumns = 1;

  /* solved on 6 iterations */
  int counter = 0;

  while (++counter <= 6) {
    size_t numRows = (input.length() + numColumns - 1) / numColumns; // Ceiling division
    std::vector<std::vector<char>> matrix(numRows, std::vector<char>(numColumns, ' ')); // Fill with spaces for alignment

    for (size_t i = 0; i < input.size(); ++i) {
      size_t row = i % numRows;
      size_t col = i / numRows;
      matrix[row][col] = input[i];
    }

    //print2DVector(matrix);

    /***
     * Replace the counter with this user choice to loop by columns,
     * and don't forget to print the vector
     */
     //char choice;
     //std::cout << "Continue with the next iteration? (y/n): ";
     //std::cin >> choice;
     //if (choice != 'y') {
     //  result = matrix;
     //  break;
     //}

    if (counter == 6) {
      result = matrix;
      break;
    }

    ++numColumns;
  }

  result = rearrangeColumns(result);
  //print2DVector(result);
  return buildStringFrom2DArray(result);
}

#endif //P0000_H